...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Svsuvolunteer account.<br>
        ...
        &mdash; Svsuvolunteer Team
      `
      return sendMail({ toEmail: email, subject: 'Svsuvolunteer - Password Reset', content })
    })
